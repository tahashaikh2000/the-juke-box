...
public class SongDBoperation {
    ...
    int getAlbumId(String albumname){
         try{
             ...
             if(rs.next()){
                 ...
                 newAlbumId=addAlbumId(albumname);
             }
             con.close();
             ps.close();
         }
    ...
    public int addAlbumId(String albumname){
         if(newAlbumId==0){
             try{
                 Class.forName("com.mysql.cj.jdbc.Driver");
                 Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/Jukebox", "root", "Arm@1234");
                 String query = "insert into album (albumname) values(?)";
                 PreparedStatement ps = con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
                 ps.setString(1,albumname);
                 //ps.setString(2,"2020");
                 int result = ps.executeUpdate();
    ...
    int getGenreId(String genrename){
         try{
             ...
             if(rs.next()){
                 ...
                 newGenreId=addGenreId(genrename);
             }
             con.close();
             ps.close();
         }
    ...
    public boolean addSong(String songname, String songsduration, String artist, String albumName, String genre){
        ...
        if(songId==0){
            try{
                ...
                pst.setInt(4,albumId);
                pst.setInt(5,genreId);
